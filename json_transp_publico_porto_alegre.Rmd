---
title: "Paths linhas de transporte pÃºblico em Porto Alegre"
output:
  html_document:
    df_print: paged
---
...
linha = 5983
```
Criamos o path para o json com as coordenadas:
```{r}
linha = 5983
...
Visualizamos o registro:
```{r}
head(json_file)
```


Recuperamos os atributos:
```{r}
json_file$idlinha
json_file$nome
json_file$codigo
...
Também podemos trabalhar com listas:
```{r}
bb = fromJSON(path)
bb
class(bb)
...
Realizamos um iteração com as coordenadas geográficas
```{r}
...
O próximo pass será jogar estas coordenas no Open Street Maps:  
.....  
....  
...  
..  
...
##Paradas  
Carregamos os dados da bae oficial:
```{r}
## paradas
...
Plotamos as paradas e a linha originalmente vista acima:  
```{r}
